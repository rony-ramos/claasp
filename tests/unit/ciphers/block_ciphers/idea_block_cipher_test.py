...
def test_idea_cipher_components():
    ...
    # Round 1 should start with MODMUL (first cipher round)
    first_cipher_component = idea.component_from(1, 0)
    assert first_cipher_component.type == "word_operation"
    assert first_cipher_component.description[0] == "MODMUL"
    ...
    # IDEA should use MODMUL, MODADD, XOR, and ROTATE (for key schedule)
    assert "MODMUL" in component_types
...
def test_idea_cipher_full_rounds():
    ...
    # Count MODMUL operations: each cipher round has 4 MUL operations
    # 8 rounds × 4 = 32, plus 2 more in output transformation = 34 total
    all_components = idea.get_all_components()
    modmul_components = [c for c in all_components if c.type == "word_operation" and c.description[0] == "MODMUL"]
    
    # Each round: 2 MODMUL (X1*Z1, X4*Z4) + 2 MODMUL in MA box (T1*Z5, U2*Z6) = 4 per round
    # 8 rounds × 4 = 32, plus 2 in output transformation (X1*Z1, X4*Z4) = 34 total
    assert len(modmul_components) == 34, f"Expected 34 MODMUL components, got {len(modmul_components)}"
...
def test_idea_cipher_component_ids():
    ...
    # Round 1 is first cipher round with MODMUL
    first_cipher_component = idea.component_from(1, 0)
    assert first_cipher_component.id.startswith("modmul_1_")
...
def test_idea_cipher_official_vector():
    ...
    # Official test vector from the IDEA specification (Appendix B)
    plaintext = 0xf129a6601ef62a47
    key = 0x2bd6459f82c5b300952c49104881ff48
    expected_ciphertext = 0xea024714ad5c4d84

    actual_ciphertext = cipher.evaluate([plaintext, key])

    assert actual_ciphertext == expected_ciphertext, (
        f"Official test vector mismatch: expected {hex(expected_ciphertext)}, got {hex(actual_ciphertext)}"
    )
